...
func _get_exported_parameters() -> Dictionary[StringName, Dictionary]:
	...
	return {
		position = CtrlrHelper.get_vec2_controller_args([], get_prop(&"position", frame)),
		rotation_degrees = CtrlrHelper.get_float_controller_args([], false, get_prop(&"rotation_degrees", frame), -INF, INF, .001, .2),
		scale = CtrlrHelper.get_vec2_controller_args([], get_prop(&"scale", frame)),
		skew = CtrlrHelper.get_float_controller_args([], false, get_prop(&"skew", frame)),
