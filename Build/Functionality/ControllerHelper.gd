...
static func string_args(val: String, controller_type: IS.StringControllerType = 0, open_extensions: Array[String] = [], placeholder: String = "") -> Array:
	return [val, placeholder, controller_type, open_extensions]

static func int_args(val: int, min: int = -INF, max: int = INF, step: int = 1, spin_scale: int = 1, spin_magnet_step: int = 10, controller_type: IS.FloatControllerType = 0) -> Array:
	return [val, min, max, step, spin_scale, spin_magnet_step, true, controller_type]

static func options_args(val: int, options: Dictionary) -> Array:
	return [val, -INF, INF, 1, 1, 1, true, IS.FloatControllerType.TYPE_OPTIONS, options]

static func float_args(val: float, min: float = -INF, max: float = INF, step: float = .01, spin_scale: float = .01, magnet_step: float = 10., controller_type: IS.FloatControllerType = 0) -> Array:
	return [val, min, max, step, spin_scale, magnet_step, false, controller_type]
...
#static func get_callable_args(name: StringName, method: Callable) -> Dictionary[StringName, Variant]:
...
#
#static func get_string_controller_args(ui_cond: Array, text: String = "", multiline: bool = false, open_file_system_mode: int = 0, open_extensions: Array[String] = [], placeholder: String = "") -> Dictionary[String, Variant]:
	#return {'placeholder': placeholder, 'val': text, 'multiline': multiline, 'open_file_system_mode': open_file_system_mode, 'open_extensions': open_extensions, 'ui_cond': ui_cond}
#
...
#
#static func get_list_controller_args(ui_cond: Array, list: Array = [], list_types: Array[StringName] = [], can_add_element: bool = true, can_remove_element: bool = true, can_duplicate_element: bool = true, can_change_element_priority: bool = true, min_elements_count: int = 0) -> Dictionary[String, Variant]:
	#return {'val': list, 'list_types': list_types, 'connections': can_add_element, 'can_remove_element': can_remove_element, 'can_duplicate_element': can_duplicate_element, 'can_change_element_priority': can_change_element_priority, 'min_elements_count': min_elements_count, 'ui_cond': ui_cond}
#
