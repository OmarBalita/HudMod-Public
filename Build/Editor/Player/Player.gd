...
var texture_pause = preload("res://Asset/Icons/pause.png")
var texture_ratio = preload("res://Asset/Icons/aspect-ratio.png")
var texture_full_screen = preload("res://Asset/Icons/expand.png")
var texture_cancel_full_screen = preload("res://Asset/Icons/cancel-expand.png")
...
@export var draw_editor:= DrawEdit.new()
@export var draw_editor_control:= DrawEditControl.new()
...
func _ready() -> void:
	...
	options_container.add_child(IS.create_empty_control())
	options_container.add_child(play_button)
	options_container.add_child(IS.create_v_line_panel())
	options_container.add_child(time_panel)
	options_container.add_child(IS.create_empty_control(10, 10, {size_flags_horizontal = Control.SIZE_EXPAND_FILL}))
	...
	time_slider = IS.create_slider_control(curr_frame, 0, ProjectServer.length, 1)
...
func update_timecode() -> void:
	...
	var video_length_timecode = TimeServer.frame_to_timecode(ProjectServer.length)
...
func update_object_editor() -> void:
	...
	if focus:
		var focus_metadata = focus.metadata
		var object = Scene.get_scene_node(focus_metadata.layer_index)
		...
		var is_frame_entered_media = ProjectServer.is_frame_on_media(curr_frame, focus_metadata.clip_pos, focus_metadata.clip_res.length)
		if object_editor:
