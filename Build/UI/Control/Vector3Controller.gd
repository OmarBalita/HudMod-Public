...
func _ready() -> void:
	...
	IS.add_children(self, [
		x_edit, IS.create_color_rect(Color(Color.RED, .5), {custom_minimum_size = Vector2(.0, 2.)}),
		y_edit, IS.create_color_rect(Color(Color.GREEN, .5), {custom_minimum_size = Vector2(.0, 2.)}),
		z_edit, IS.create_color_rect(Color(Color.BLUE, .5), {custom_minimum_size = Vector2(.0, 2.)})
	])
...
func set_suffix(type: FloatController.SuffixType, color: Color = Color(IS.color_label, 0.5)):
	...
	z_edit.set_suffix_by_type(type, color)

func set_prefix(text: String, color: Color = Color(IS.color_label, 0.5)):
	x_edit.set_prefix(text, color)
	y_edit.set_prefix(text, color)
	z_edit.set_prefix(text, color)
