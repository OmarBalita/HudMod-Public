...
@export var focus_close_dist: float = 2.0
@export var navigate_dist: float = 50.0
@export var navigate_speed: float = 50.0
@export_group("Font")
@export var font: Font = preload("res://Asset/Fonts/Cascadia.ttf")

var curves_profiles: Array[CurveSampler.Profile]
...
func find_focused_keys_index(coord: Vector2) -> int:
	var result: Dictionary[int, float] = {-1: INF}
	for keys_index: int in curves_profiles.size():
		var profile: CurveSampler.Profile = curves_profiles[keys_index]
		var x1: float = coord.x
		var x2: float = coord.x + .01
		var y1: Variant = profile.sample(x1)
		var y2: Variant = profile.sample(x2)
		var slope: float = abs(y2 - y1) / abs(x2 - x1)
		var dynamic_focus_close_dist: float = focus_close_dist * (1.0 + slope)
		var y_diff: float = abs(y1 - coord.y)
...
func get_domain_from_display_pos(pos: float) -> float:
	return clamp(snapped(min_domain + pos * (max_domain - min_domain) / size.x, domain_step), min_domain, max_domain)
...
func navigate_value(offset: float) -> void:
	min_val += offset
	max_val += offset
	udpate_selectable_points_display_poss(func(key: float, point_info: LocalPointInfo) -> Vector2:
		return get_display_pos_from_coord(Vector2(key, point_info.point_val.value))
	)
	...
	if event is InputEventMouse:
		...
		if event is InputEventMouseButton:
			...
			if is_pressed and event.button_index in [MOUSE_BUTTON_LEFT, MOUSE_BUTTON_RIGHT]:
				...
				if not is_control_finded and motion_mode not in [1, 2] and curr_focused_keys_index > -1:
			...
			if not is_key_finded: focused_keys_index = find_focused_keys_index(coord)
...
func _input_move_selected_points(coord: Vector2, mouse_pos: Vector2) -> void:
	...
	for object: Variant in selected_points:
		...
		for key: float in object_selected_points:
			if not object_selected_points.has(key):
				continue
...
func _draw() -> void:
	...
	if is_snapped:
		...
		var y_snap_steps_range: Array = range(-3, val_size / draw_step.y + 3)
	...
	for keys_index: int in curves_profiles.size():
		...
		var keys_color: Color = Color(KEYS_COLORS[keys_index], color_alpha)
...
func _update_curves_profiles() -> void:
	curves_profiles.clear()
	for time: int in keys_type + 1:
		curves_profiles.append(CurveSampler.create_profile(0, {}))
		add_selectable_object(time, {})
